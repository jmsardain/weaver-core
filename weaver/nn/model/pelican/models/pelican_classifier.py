...
class PELICANClassifier(nn.Module):
    ...
    def __init__(self, num_channels_scalar, num_channels_m, num_channels_2to2, num_channels_out, num_channels_m_out, num_classes=2,
                 ...
                 device=torch.device('cuda'), dtype=None,
                 #weaver misc
                 trim=True,
                 for_inference=False,
                 use_amp=False,
        ...
        self.input_encoder = InputEncoder(embedding_dim, device = self.device, dtype = dtype)
        ...
        if self.num_scalars > 0:
            self.eq1to2 = Eq1to2(self.num_scalars, num_channels_scalar, activate_agg=activate_agg_in, activate_lin=activate_lin_in, activation = activation, average_nobj=average_nobj, config=config_out, factorize=False, device = self.device, dtype = dtype)
        ...
        self.net2to2 = Net2to2(num_channels_2to2 + [num_channels_m_out[0]], num_channels_m, activate_agg=activate_agg, activate_lin=activate_lin, activation = activation, dropout=dropout, drop_rate=drop_rate, batchnorm = batchnorm, config=config, average_nobj=average_nobj, factorize=factorize, masked=masked, device = self.device, dtype = dtype)
        ...
        self.msg_2to0 = MessageNet(num_channels_m_out, activation=activation, batchnorm=batchnorm, device=self.device, dtype=dtype)       
        self.agg_2to0 = Eq2to0(num_channels_m_out[-1], num_channels_out[0] if mlp_out else num_classes, activate_agg=activate_agg_out, activate_lin=activate_lin_out, activation = activation, config=config_out, factorize=False, average_nobj=average_nobj, device = self.device, dtype = dtype)
        ...
        if mlp_out:
            self.mlp_out = BasicMLP(self.num_channels_out + [num_classes], activation=activation, dropout = False, batchnorm = False, device=self.device, dtype=dtype)
