...
class Eq2to0(nn.Module):
    ...
    def forward(self, inputs, mask=None, nobj=None, irc_weight=None):
        ...
        output = torch.einsum('dsb,ndb->ns', coefs.to(torch.device("cuda")), ops)

        output = output + self.bias.to(torch.device("cuda"))
...
class Eq2to2(nn.Module):
    ...
    def forward(self, inputs, mask=None, nobj=None, softmask_ir=None, irc_weight=None):
        ...
        if self.factorize:
            ...
            coefs = self.coefs
        output = torch.einsum('dsb,ndbij->nijs', coefs.to(torch.device("cuda")) , ops)

        diag_eye = torch.eye(inputs.shape[1], device=self.device, dtype=self.dtype).unsqueeze(0).unsqueeze(-1)
        diag_bias = diag_eye.multiply(self.diag_bias.to(torch.device("cuda")).view(1,1,1,-1))
        output = output + self.bias.to(torch.device("cuda")).view(1,1,1,-1) + diag_bias
