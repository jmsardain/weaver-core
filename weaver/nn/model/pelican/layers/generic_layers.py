...
class BasicMLP(nn.Module):
    ...
    def __init__(self, num_channels, activation='leakyrelu', ir_safe=False, batchnorm=False, dropout=False, drop_rate=0.25, device=torch.device('cuda'), dtype=torch.float):
    ...
    def forward(self, x, mask=None):
        ...
        for (lin, activation) in zip(self.linear, self.activations):
            x = activation.to(torch.device("cuda"))(lin.to(torch.device("cuda"))(x))
...
class MessageNet(nn.Module):
    ...
    def __init__(self, num_channels, depth=2, activation='leakyrelu', ir_safe=False, batchnorm = None, masked=True, device=torch.device('cuda'), dtype=torch.float):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        if self.batchnorm: 
            if self.batchnorm.startswith('b') or self.batchnorm.startswith('i'):
                if len(x.shape)==3:
                    if self.masked:
                        x = self.normlayer.to(torch.device("cuda"))(x.unsqueeze(1), mask).squeeze(1)
                    else:
                        x = self.normlayer.to(torch.device("cuda"))(x.unsqueeze(-1).permute(0,2,1,3)).permute(0,2,1,3).squeeze(-1)
                elif len(x.shape)==4:
                    if self.masked:
                        x = self.normlayer.to(torch.device("cuda"))(x, mask)
                    else:
                        x = self.normlayer.to(torch.device("cuda"))(x.permute(0,3,1,2)).permute(0,2,3,1)
            elif self.batchnorm.startswith('l'):
                if len(x.shape)==3:
                    if self.masked:
                        x = self.normlayer.to(torch.device("cuda"))(x.unsqueeze(1).unsqueeze(1), mask.unsqueeze(1).unsqueeze(1)).squeeze(1).squeeze(1)
                    else:
                        x = self.normlayer.to(torch.device("cuda"))(x.unsqueeze(1)).squeeze(-1)
                elif len(x.shape)==4:
                    if self.masked:
                        x = self.normlayer.to(torch.device("cuda"))(x.unsqueeze(1), mask.expand(x.shape).unsqueeze(1)).squeeze(1)
                    else:
                        x = self.normlayer.to(torch.device("cuda"))(x.unsqueeze(1)).squeeze(1)
...
class InputEncoder(nn.Module):
    def __init__(self, out_dim, device=torch.device('cuda'), dtype=torch.float):
        super().__init__()
        self.device = device
        ...
        self.zero = torch.tensor(0, device=self.device, dtype=dtype)
    ...
    def forward(self, x, mask=None, mode='log'):
        ...
        if mode=='log':
            x = ((1 + x).abs().pow(1e-6 + self.alphas.to(torch.device("cuda")) ** 2) - 1) / (1e-6 + self.alphas.to(torch.device("cuda")) ** 2)

        if mode=='angle':
            x = ((1e-5 + x.abs()).pow(1e-6 + self.alphas.to(torch.device("cuda")) ** 2) - 1) / (1e-6 + self.alphas.to(torch.device("cuda")) ** 2)

        if mode=='arcsinh':
            x = (x * 2 * self.alphas.to(torch.device("cuda"))).arcsinh() / (1e-6 + self.alphas.to(torch.device("cuda")).abs())
